...
while True:
    ...
    if makeplots:
        ...
        Grid2D = np.array( [xx.reshape(Nplot**2), yy.reshape(Nplot**2), np.ones(Nplot**2)*0.5*Boxsize] ).T
        ...
        pc  = ax.pcolormesh( Edges1d, Edges1d, Density[cells].reshape((Nplot,Nplot)), rasterized=True, cmap=plt.get_cmap('viridis'), vmin=0, vmax=70. )
